...
class OpenHSIPushPullSystem:
    ...
    def create_minimal_calibration(self, cal_path: Path):
        ...
        self.calibrate = self.cam_props.cal
    ...
    def calibrate_with_openhsi_method(self, calibration_dir: str = "calibration_data"):
        ...
        try:
            ...
            try:
                hsi_io.save_calibration(cal_dir / "dark_reference.npy", self.calibration_targets["dark"])
            except:
            ...
            try:
                hsi_io.save_calibration(cal_dir / "white_reference.npy", self.calibration_targets["white"])
            except:
            ...
            try:
                hsi_io.save_calibration(cal_dir / "foil_reference.npy", self.calibration_targets["foil"])
            except:
    ...
    def compute_calibration_coefficients(self, cal_dir: Path):
        ...
        wavelengths = self.cam_props_cal.get("wavelengths", None)
    ...
    def convert_to_reflectance(self, radiance_cube):
        ...
        """
        try:
            # Method 0: Openhsi calibration
            if ELC in openhsi:
                elc = ELC(
                    cube=radiance_cube,
                    white_ref=self.calibrate_targets["white"],
                    dark_ref=self.calibrate_targets["dark"]
                )
                reflectance = elc.apply()
                return reflectance
    ...
    def run_complete_acquisition(self, sample_name: str = None):
        ...
        if self.calibration is None or 'calibration_notes' in self.calibration and 'PLACEHOLDER' in self.calibration[
            'calibration_notes']:
