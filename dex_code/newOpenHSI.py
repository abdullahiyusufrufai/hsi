...
from openhsi.data import CameraProperties, load_calibration
...
class OpenHSIPushPullSystem:
    ...
    def create_minimal_calibration(self, cal_path: Path):
        ...
        cal_data = {
            ...
            "dark_current": np.zeros((480, 224), dtype=np.float32),
            "flat_field": np.ones((480, 224), dtype=np.float32),
            "rad_fit": np.ones((224,), dtype=np.float32) * 100.0,  # Placeholder
            "smile_correction": np.zeros((480, 224), dtype=np.float32),
            "calibration_date": datetime.now().isoformat(),
            "calibration_notes": "PLACEHOLDER - RUN PROPER CALIBRATION"
        }
        settings_path = self.config_dir / "settings.json"
        cam_props = CameraProperties(json_path=str(settings_path))
        cam_props.cal = cal_data
        cam_props.dump_calibration(str(cal_path))
    ...
    def calibrate_with_openhsi_method(self, calibration_dir: str = "calibration_data"):
        ...
        try:
            ...
            np.save(cal_dir / "dark_reference.npy", dark=self.calibration_targets["dark"])
            ...
            np.save(cal_dir / "white_reference.npy", white=self.calibration_targets["white"])
            ...
            np.save(cal_dir / "foil_reference.npy", foil=self.calibration_targets["foil"])
    ...
    def compute_calibration_coefficients(self, cal_dir: Path):
        ...
        cam_props = CameraProperties(json_path=str(settings_path))
        cam_props.cal = cal_data
        cam_props.dump_calibration(str(cal_dir / "computed_calibration.json"))
    ...
    def save_openhsi_format(self, sample_name: str = None):
        ...
        try:
            ...
            if self.reflectance_cube is not None:
                ...
                cube=self.reflectance_cube,
                cam_props=self.cam_props,
                metadata=metadata
                )

                reflectance_path = output_dir / f"{sample_name}_reflectance.nc"
                reflectance_dc.save(reflectance_path)
